---
title: Одноразовые пароли доступа по ssh через HashiCorp Vault
tags:
  - vault
  - ssh
  - туториал
...
централизованный и защищённый доступ к вашим машинам по ssh, через создание клиентских 
...
Создаём token и сохраняем его, потому что запросить его у vault не удастся.
...
одноразовых ключей для всех пользователй и всех машин, которые указаны в 
роли. **Поэтому рекомендуется ограничивать управление роли одним пользователем на одной машине**.

#### #2 Сгенерированные ключи останутся рабочими даже после удаления роли
...
#### #4 Инвалидировать выданные ключи можно только инвалидировав привязанный к ним lease

Так как ключи живут даже после удаления роли, единственный способ их убить, это инвалидировать lease.

Можно инвалидировать точечно:
```sh
...
#### Пример с использованеим API запросов
